...
require('./config')
const { default: makeWASocket, useMultiFileAuthState, DisconnectReason, fetchLatestBaileysVersion, makeInMemoryStore, jidDecode, proto, getContentType, downloadContentFromMessage, fetchLatestWaWebVersion, getAggregateVotesInPollMessage } = require("@adiwajshing/baileys");
const fs = require("fs");
...
console.log(chalk.red('Offmon - Multi Device • Ziyo Official'))
...
if(usePairingCode && !ptz.authState.creds.registered) { console.log(chalk.magenta.bold('\nMasukan Nomor Yang Aktif :'))
...
async function getMessage(key){
    ...
    return {
        conversation: "Hai Im Offmon Bot"
    }
}
...
ptz.ev.on('messages.update', async chatUpdate => {
  /*  if (chatUpdate.messages && chatUpdate.messages[0].type === 'text') {
    const messageId = chatUpdate.messages[0].id; // Get the message ID
    const responseMessage = 'This is the automatic response'; // Your response message

    // Store the message-response pair
    messageResponses[messageId] = responseMessage;

    // Send the message
    await ptz.sendMessage(chatUpdate.messages[0].chat, { text: chatUpdate.messages[0].content.text });
  }*/
    for(const { key, update } of chatUpdate) {
        if(update.pollUpdates && key.fromMe) {
            ...
            if(pollCreation) {
                ...
                // Memastikan ada pembaruan dengan pemilih
                const chosenOption = pollUpdate.filter(v => v.voters.length !== 0)[0]?.name
                
                if (chosenOption !== undefined) {
                    let responseMessage = ''

                    // Merespons berdasarkan pilihan
                    if (chosenOption.toLowerCase() === 'ya') {
                        responseMessage = 'tentu'
                    } else if (chosenOption.toLowerCase() === 'tidak') {
                        responseMessage = 'kenapa'
                    }

                    if (responseMessage) {
                        // Kirim pesan respon ke chat
                        ptz.sendMessage(key.remoteJid, { text: responseMessage })
                    }
                }
            }
        }
...
if (reason === DisconnectReason.badSession) { console.log(`Bad Session File, Please Delete Session and Verifikasi Again`); process.exit(); }
